...
impl EventListener {
    ...
    pub async fn start_listener_async(&mut self) -> crate::Result<()> {
        ...
        let mut buf = String::with_capacity(INITIAL_BUF_CAPACITY);
        loop {

            let num_read = reader.read_line(&mut buf).await?;
            if num_read == 0 {
                break;
            }
            let parsed: Vec<Event> = event_parser(&buf)?;
            buf.clear();
